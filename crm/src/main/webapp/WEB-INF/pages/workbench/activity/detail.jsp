...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.code == "1") {
                            ...
                            // htmlStr+="        <h5>"+noteContent+"</h5>";用局部变量也可以
                            htmlStr += "        <h5 id=\"h5_" + data.retData.id + "\">" + data.retData.noteContent + "</h5>";//用返回来的值
            ...
            //给所有市场活动备注后面的修改图标添加单击事件
            $("#remarkDivList").on("click", "a[name=\"editA\"]", function () {
                //获取备注id和noteContent
                var id = $(this).attr("remarkId");
                var noteContent = $("#div_" + id + " h5").text();
                //把id和noteContent写入修改市场活动备注的模态窗口
                $("#edit-id").val(id);
                ...
                $.ajax({
                    ...
                    success:function (data){
                        if (data.code=="1"){
                            ...
                            //修改 时间 人
                            $("#div"+data.retData.id+" small").text(" "+data.retData.editTime+" 由${sessionScope.sessionUser.name}修改");
                        }else {
                            alert(data.message );
                            $("#editRemarkModal").modal("show");
                        }
...
<div id="remarkDivList" style="position: relative; top: 30px; left: 40px;">
    ...
    <%--                    &lt;%&ndash;	<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;"> <c:if test="${remark.editFlag=='1'}">${remark.editTime}</c:if> <c:if test="${remark.editFlag!='1'}">${remark.createTime}</c:if> 由zhangsan</small>&ndash;%&gt;--%>
