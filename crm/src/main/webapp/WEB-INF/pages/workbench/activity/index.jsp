    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $(".mydate").datetimepicker({
                ...
                todayBtn: true,//设置是否现实“今天”按钮，默认为false
            ...
            $("#exportActivityXzBtn").click(function () {
                ...
                ids = ids.substring(0, ids.length - 1);
                //    同步请求url：window.location.href = "workbench/activity/exportActivities.do?" + idsStr;  可以达成，但是url长度有限
                // window.location.href = "workbench/activity/exportActivities.do?" + ids;
                // ajax同步请求  不能达成  ajax函数的返回类型只有xml、text、json、html等类型  不能下载
                // $.ajax({
                //     url:'workbench/activity/exportActivities.do',
                //     type: 'post',
                ...
                // 模拟表单提交同步方式下载文件       可以达成 字符串拆串实现    表单 ids字符串 不能变成  数组
                var form = $("<form></form>").attr("action", "workbench/activity/exportActivities.do").attr("method", "post");
                form.append($("<input></input>").attr("type", "hidden").attr("name", "ids").attr("value", ids));
                form.appendTo('body').submit().remove()


                // postExcelFile({data:ids} , 'workbench/activity/exportActivities.do');
            });
            //给导入按钮添加单击事件
            $("#importActivityBtn").click(function () {
                //收集参数
                var activityFileName = $("#activityFile").val();
                // activityFileName.substring(activityFileName.length-4,activityFileName.length)
                ...
                //FormData是ajax提高的接口，可以模拟键值对向后台提交参数；
                //FormData最大的优势是不但能提交文本数据，而且还能提交二进制数据
                var formData = new FormData();
                formData.append("activityFile", activityFile);
                formData.append("userName", "666");
                //发送请求
                $.ajax({
                    url: 'workbench/activity/importActivity.do',
                    data: formData,
                    processData: false,//设置ajax向后台提交参数之前，是否把参数统一转换成字符串：true--转（默认），false不转
                    contentType: false,//设置ajax向后台提交参数之前，是否把所有参数统一按urlencoded编码：true（默认），false
                    type: 'post',
                    dataType: 'json',
                    success: function (data) {
                        if (data.code == "1") {
        ...
        function queryActivityByConditionForPage(pageNo, pageSize) {
            ...
            //收集参数,
            //查询的时候不需要去空格，不需要表单验证，她不是添加不会破坏表单
